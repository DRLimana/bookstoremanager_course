...
import com.dlimana.bookstoremanager.users.service.JwtTokenManager;
...
public class JwtRequestFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws ServletException, IOException {
        ...
        if(isTokenPresent(requestTokenHeader)){
            //da posicao 7 em diante vai conter o token jwt
            jwtToken = requestTokenHeader.substring(7);
            username = jwtTokenManager.getUsernameFromToken(jwtToken);
        }else{
            logger.warn("JWT Token does not begin with Bearer String");
        }
        //verificar se o username for preenchido e se o object security context holder for vazio, ele vai adicionar o contexto
